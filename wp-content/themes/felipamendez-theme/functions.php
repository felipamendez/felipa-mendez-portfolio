...
//load more button with template variable and if statements
function load_more() {
	//make a variable, pull either project or blog, pass as a arg
	...
	$template = $ajaxposts->get('template');

	if($ajaxposts->have_posts()) {
		while($ajaxposts->have_posts()) : $ajaxposts->the_post();

				$response .= '<article class="row mb-5 mb-md-6 mx-0 gx-0 gx-lg-5 gy-3 align-items-center">';
					$response .= '<div class="col-12 col-lg-4">';
						$response .= '<a href="' . get_permalink() . '">';
							$response .= '<img src="' . get_field('card_img') . '" class="card-img rounded">';
						$response .= '</a>';
					$response .= '</div>';
					$response .= '<div class="col-12 col-lg-8">';
						$response .= '<h2 class="mb-3" style="font-size:1.8rem;"><a href="' . get_permalink() . '"> ' . get_the_title() . '</a></h2>';
						$response .= '<div class="mb-3">';
						$post_tags = get_the_tags();
						if ($post_tags) {
		                    foreach ($post_tags as $tag) {
		                        $response .= "<span class='pills px-sm-2 py-sm-1 rounded' style='margin-right:10px;color:#005A8B;background-color:#E9EEFF;'>" . $tag->name . "</span>";
		                    }
		                };
						$response .= '</div>';
						$response .= '<p class="mb-0"><a href="' . get_permalink() . '"><strong>Read More</strong></a></p>';
					$response .= '</div>';
				$response .= '</article>';
...
function tag_filter() {
	...
	$arr_posts = new WP_Query([
		...
		'template' => $_POST['template'],
						...
						$response = '';
						$max_pages = $arr_posts->max_num_pages;

						$template = $arr_posts->get('template');
						...
						if($arr_posts->have_posts()) {
							while($arr_posts->have_posts()) : $arr_posts->the_post();
									...
									$response .= '<article class="col-12 col-lg-4 col-md-6 align-items-center">';
										$response .= '<div class=" pb-6">';
											...
											$response .= '<div class="px-lg-3 px-2">';
												$response .= '<a href="' . get_permalink() . '">';
													$response .= '<h2 class="h5 mb-2" style="font-size:1.8rem;">' . get_the_title() . '</h2>';
												$response .= '</a>';
												$response .= '<div class="mb-4">';
													$post_tags = get_the_tags();
													if ($post_tags) {
														foreach ($post_tags as $tag) {
															$response .= "<span class='pills px-sm-2 py-sm-1 rounded' style='cursor:pointer;margin-right:10px;color:#005A8B;background-color:#E9EEFF;'>" . $tag->name . "</span>";
														}
													};
												$response .= '</div>';
												
												$response .= '<p class="mb-0">';
													$response .= '<a href="' . get_permalink() . '"><strong>Read More</strong></a>';
												$response .= '</p>';
											$response .= '</div>';
										$response .= '</div>';
									$response .= '</article>';
								...
								$response .= '<article class="row mb-5 mb-md-6 mx-0 gx-0 gx-lg-5 gy-3 align-items-center">';
									$response .= '<div class="col-12 col-lg-4">';
										$response .= '<a href="' . get_permalink() . '">';
											$response .= '<img src="' . get_field('article_card_image') . '" class="card-img rounded">';
										$response .= '</a>';
									$response .= '</div>';
									$response .= '<div class="col-12 col-lg-8">';
										$response .= '<h4 class="mb-3"><a href="' . get_permalink() . '"> ' . get_the_title() . '</a></h4>';
										$response .= '<div class="mb-3">';
										$post_tags = get_the_tags();
										if ($post_tags) {
											foreach ($post_tags as $tag) {
										...
										if ($_POST['category'] != 'Webinar') : 
											$response .= '<p>' . get_the_date() . '</p>';
										endif;
				
										$response .= '<p class="mb-3">' . get_field('article_text_blurb') . '</p>';
										$response .= '<p class="mb-0"><a href="' . get_permalink() . '"><strong>Read More</strong></a></p>';
									$response .= '</div>';
								$response .= '</article>';
							}
...
add_action('wp_ajax_load_more_tag_filter', 'load_more_tag_filter');
add_action('wp_ajax_nopriv_load_more_tag_filter', 'load_more_tag_filter');



//resets filter for posts
function remove_tag_filter() {
...
add_action('wp_ajax_remove_tag_filter', 'remove_tag_filter');
add_action('wp_ajax_nopriv_remove_tag_filter', 'remove_tag_filter');